...
def update_passwords_to_hash():
    ...
    for row in rows:
        ...
        cur.execute(
            "UPDATE Users SET Password = %s WHERE User_ID = %s", (hashed_password,))
        conn.commit()
...
def menu():
    while True:
        ...
        if choice == "1":
            ...
            if logged_in_user_level == 0 or logged_in_user_level == 1:
                # asks for details then calls record_sale(...) for insert into db
                add_sale
        elif choice == "5":
            exit()
...
def list_cars():
    ...
    try:
        ...
        for row in rows:
            print(
                f"VIN: {row[0]}, Make: {row[1]}, Color: {row[2]}, Model: {row[3]}, Year: {row[4]}, Starting Price: {row[5]}, Is Sold: {row[6]}")
    except psycopg2.Error as e:
...
def list_customer():
    try:
        cur.execute(
            "SELECT Customer_ID, First_Name, Last_Name, Email_Address, Phone_Number FROM customers")
        rows = cur.fetchall()
        if rows:
            ...
            for row in rows:
                print(
                    f"{row[0]:<5} {row[1]} {row[2]:<20} {row[3]:<30} {row[4]:<15}")
        else:
